...
The following headers are sent to each Now deployment and can be used to process the request before sending back a response. These headers can be read from [Request](https://nodejs.org/api/http.html#http_message_headers) object in your programming language of choice.
...
This header represents the domain name of the forwarded server.
...
This header represents the unique deployment, not the alias.
...
In addition to regular [HTTP headers](https://developer.mozilla.org/en-US/docs/Web/HTTP/Headers) and Now headers, it is also possible to supply your own custom response headers.

Custom headers are added in the same way regular headers are, by making an addition to the [routes](/docs/v2/deployments/routes/#headers) property in your `now.json` file.
